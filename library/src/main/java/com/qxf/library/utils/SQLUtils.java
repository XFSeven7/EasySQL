...
//            resultSQL.add(dropSQL);// TODO 删除字段 还没有找到很好的方法来解决这个问题 = =！
	 ...
	 * @return 是否存在
	 */
	public static boolean hasTable(Class<? extends EasyTable> classzz, DBHelper dbHelper) {

		ArrayList<String> strings = dbHelper.tableList();
		for (String table : strings) {

			if (TextUtils.equals(table, classzz.getSimpleName())) {
				return true;
			}
